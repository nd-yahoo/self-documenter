...
import { parseCSV, validateCSV } from '../utils/csvUtils';
...
export async function processCSVWithScreenshots(
  ...
  for (let i = 0; i < uniqueQueries.length; i++) {
    sendProgressUpdate(
      `Processing comparison ${i + 1} of ${uniqueQueries.length}...`,
      Math.round((i / uniqueQueries.length) * 100)
    );
    ...
    await createComparisonScreenshots(
      card, 
      query, 
      yahooScreenshotsByQuery, 
      competitorScreenshotsByQuery,
      competitorEngine,
      SCREENSHOT_WIDTH,
      SCREENSHOT_HEIGHT,
      UI_CONSTANTS.CARD_PADDING,
      SCREENSHOT_SPACING
    );
    
    // Add metadata 
    await createTextNode(
      `Engine: Yahoo vs ${competitorEngine.charAt(0).toUpperCase() + competitorEngine.slice(1)} | Mode: ${deviceMode.charAt(0).toUpperCase() + deviceMode.slice(1)}`,
      {
        fontSize: 12,
        x: UI_CONSTANTS.CARD_PADDING,
    ...
    if (additionalColumns.length > 0) {
      ...
      if (matchingDataRow) {
        await addAdditionalData(card, matchingDataRow, additionalColumns, SCREENSHOT_HEIGHT, UI_CONSTANTS.CARD_PADDING);
      }
    }
  }
 ...
 */
async function createComparisonScreenshots(
  card: FrameNode,
  query: string,
  yahooScreenshotsByQuery: Map<string, ScreenshotData>,
  competitorScreenshotsByQuery: Map<string, ScreenshotData>,
  competitorEngine: string,
  width: number,
  height: number,
  padding: number,
  ...
  if (yahooScreenshot) {
    try {
      ...
      await createTextNode(yahooScreenshot.filename, {
        fontSize: 8,
        x: 5,
        y: height - 15,
        fill: { r: 0.5, g: 0.5, b: 0.5 },
      ...
      await createTextNode(`Error loading Yahoo screenshot:\n${err.message || 'Unknown error'}`, {
        fontSize: 12,
        x: 10,
        y: height / 2 - 20,
        width: width - 20,
    ...
    await createTextNode(`No Yahoo screenshot available for query:\n"${query}"`, {
      fontSize: 14,
      x: 10,
      y: height / 2 - 30,
      width: width - 20,
  ...
  await createTextNode(competitorEngine.charAt(0).toUpperCase() + competitorEngine.slice(1), {
  ...
  if (competitorScreenshot) {
    try {
      ...
      // Add small filename text at the bottom
      await createTextNode(competitorScreenshot.filename, {
        fontSize: 8,
        x: 5,
        y: height - 15,
        fill: { r: 0.5, g: 0.5, b: 0.5 },
      ...
      await createTextNode(`Error loading ${competitorEngine} screenshot:\n${err.message || 'Unknown error'}`, {
        fontSize: 12,
        x: 10,
        y: height / 2 - 20,
        width: width - 20,
    ...
    await createTextNode(`No ${competitorEngine} screenshot available for query:\n"${query}"`, {
      fontSize: 14,
      x: 10,
      y: height / 2 - 30,
      width: width - 20,
...
async function addAdditionalData(
  ...
  const additionalDataTitle = await createTextNode("Additional Data:", {
  ...
  if (additionalDataText) {
    await createTextNode(additionalDataText, {
